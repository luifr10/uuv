...
import fs from "fs";
import * as runner from "./runner-appium-uiautomator2";
import { Formatter } from "cucumber-json-report-formatter";
import path from "path";

const PROJECT_DIR = process.cwd();
const JSON_REPORT_DIR = path.join(PROJECT_DIR, "uuv/reports/e2e/json");
const HTML_REPORT_DIR = path.join(PROJECT_DIR, "./uuv/reports/e2e/html");
const CUCUMBER_MESSAGES_FILE = path.join(PROJECT_DIR, "./uuv/cucumber-messages.ndjson");
export async function main() {
  ...
  async function runE2ETests(argv: any) {
    ...
    // Running Tests
    await runner.runE2ETests(PROJECT_DIR, "uiautomator2");
  }
  ...
  function printBanner(getCurrentVersion: () => string) {
    console.log(
        chalk.blueBright(
            figlet.textSync("UUV - Appium UiAutomator2", {
              font: "Big",
...
export async function generateHtmlReport(browser: string) {
